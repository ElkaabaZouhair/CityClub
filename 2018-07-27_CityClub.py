...
#********************************************************
# Variables Globales
Version = "18.07.26.3"                  # Version Du Programme
...
boucle_maj = 300                        # utilisé comme compteur de passages pour verifier la mise a jour
...
GPIO_buzzer = 7         # Buzzer est branche sur la pin 7 / GPIO4
...
def declenchebuzzer():
    GPIO.output(GPIO_buzzer, False)     # Allumer
    # GPIO.output(GPIO_relais, True)    # Allumer
    time.sleep(0.1)        # Attendre
    # GPIO.output(GPIO_relais, False)   # Eteindre
    GPIO.output(GPIO_buzzer, True)      # Eteindre
#********************************************************
...
#Ecrire La Version Du programme dans la base
#********************************************************
def Ecrire_Version(Version_num):
    try:
        db = MySQLdb.connect("127.0.0.1", "yapo", "pipi", "rpi")
        curs=db.cursor()
        curs.execute("UPDATE Parametre SET Param_Valeur='%s' WHERE Param_Nom='Version'" % Version_num)
        # accept the changes
        db.commit()
        db.close()
        #print("Mise a jour Version Dans La Base")
...
def Ping_Yapo():
    ...
    global url_WS_tm        # Time out
    try:
        my_req = requests.get(url_yapo_ping, verify = True, timeout = timeout_ping) # Timeout
...
if True:
    ...
    while continue_reading:
        ...
        if (boucle_compteur >= 120):
            ...
            boucle_compteur = 0
        # Tous les 5 min Verifier Mise a Jour Du programme 
        boucle_maj = boucle_maj + 1
        if (boucle_maj >= 300):
            print("\nMise à jour ? : ",end="")
            boucle_maj = 0
            Reponse_ws = Get_Version_from_ws()
            if Reponse_ws != "":
                Version_ws = Reponse_ws[8:18]
                # print("Version_ws", Version_ws)
                Link = Reponse_ws[24:] # Utilise pour Telechargement
                print("PRG :", Version, "SRV :", Version_ws, "Lien :", Link)
                if Version_ws > Version:
                    print("Télécharger le programme de mise à jour",Version_ws,"\n")
                    Telecharger_Mise_a_jour_Programe(Link) #  Telécharger Programme depuis Web Service
                    time.sleep(1)
